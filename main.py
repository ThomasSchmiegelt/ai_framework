...
@app.get("/api/compare/projects/{name}")
async def compare_get(name: str):
    return _cmp_load(name)


@app.put("/api/compare/projects/{name}")
async def compare_put(name: str, req: Request):
    body = await req.json()
    return _cmp_save(name, body)


@app.delete("/api/compare/projects/{name}")
