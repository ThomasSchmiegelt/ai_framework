...
| Matrix-Recherche + **Partner-Auswertung** | `matrix_research.js`, Zellen über `/api/chat` (`science`+`use_tools`); Spalten mit `active`/`ctx` (Schalter „ausführen"/„Kontext", `_runColumn` = ▶ Spalte, Kontext-Kaskade über vorherige Spalten). **Partner-Auswertung** (`_partnerMode`): Firmenliste-Import (`_importCompanyList`, unbegrenzt), Vorlage `_PARTNER_COLUMNS` (Interesse/Profil → Kontaktdaten → LinkedIn/X/Instagram/Facebook/GitHub → Kaltakquise-Mail), Agenten idempotent angelegt via `_ensurePartnerAgents` (`partner_recherche`/`_kontakt`/`_social`/`_mail`, editierbar im Agenten-Tab); Token-Label „Partner-Auswertung" | — |
