...
| `export.py` | Generate DOCX, XLSX, PPTX from chat content (`_embed_b64_image` fits images aspect-preserving/contain into the box — no distortion) |
| `engineering.py` | Unit conversion (Pint), equation solver (SymPy), charting (matplotlib), bolt calculator (VDI 2230) |
...
### Frontend Modules (10 tabs: Chat, Canvas, Agenten, Recherche, RAG, Dokumente, Planer, Matrix, Code, Logs)
...
- `ide.js` — Code-IDE: editor + sandboxed-iframe Canvas preview, AI assistant (uses the profile **Programmieren** model via `Profile.modelFor('coding')`, fallback `ministral-3:3b`), auto-repair. Provides the `ai_framework_thomas_input()` / `ai_framework_thomas_run()` iframe framework so generated programs get interactive input fields and a responsive canvas.
...
- `make_portable` — Self-contained bundle, no system dependencies
- `make_server` — Multi-user server mode with `0.0.0.0` binding

The `make_*` scripts exclude `venv`, `__pycache__`, `.git`, `.claude`, `server.log` when copying. **Per-process VRAM guard caveat:** in `make_server` with `workers > 1`, the single-model guard does not coordinate across worker processes — on ~6 GB VRAM use `workers = 1` (documented in `docs/SERVER.md`).
