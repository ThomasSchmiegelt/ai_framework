...
- `ide.js` — Code-IDE: editor + sandboxed-iframe Canvas preview, AI assistant (uses the profile **Programmieren / Mathe** model via `Profile.modelFor('coding')` — shared with the Mathe tab, fallback `ministral-3:3b`), auto-repair. **Editor = CodeMirror 5** (MIT, loaded from jsDelivr CDN like the other libs): `CodeMirror.fromTextArea(#ide-editor)` → module var `_cm`; gives syntax highlighting, line numbers, auto-close/match brackets, active-line, autocomplete (Ctrl-Space), 2-space Tab, and the keymap Ctrl/Cmd-Enter=run, Ctrl/Cmd-S=save. **All editor access goes through `_getCode`/`_setCode`/`_setDirty`** (rerouted to `_cm`); if `window.CodeMirror` is absent (CDN offline) it **falls back to the plain textarea** path. `CodeIDE.refresh()` re-measures CM when the panel/subview becomes visible (called from `switchTab('ide')`, the code-subtab switch in `app.js`, and the splitter); theme follows the app CSS vars (`#ide-editor-panel .CodeMirror …` in `app.css`, no CM theme file). Provides the `ai_framework_thomas_input()` / `ai_framework_thomas_run()` iframe framework so generated programs get interactive input fields and a responsive canvas. The editor ↔ preview columns have a **draggable splitter** (`#ide-splitter`, `_initSplitter`; left width via the `--ide-left-w` CSS var on `#ide-body`, persisted in `localStorage`, double-click resets). (The JSON-Editor subview keeps its own textarea + gutter + live validation.)
