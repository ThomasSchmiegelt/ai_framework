...
| `/praesentation [alle\|keine\|N] <Thema>` · `/vortrag` · `/slides` | **Illustrierter Präsentationsassistent:** erzeugt eine komplette Präsentation zum Thema **inklusive KI-Bildern** (über dein eingestelltes Bildmodell) und öffnet sie im **Canvas**. Standard: Bilder auf **Titel- und Abschnittsfolien**; mit vorangestelltem **`alle`** bekommt jede Folie ein Bild, **`keine`** erzeugt nur Textfolien, eine **Zahl** begrenzt die Bildanzahl (z. B. `/praesentation 3 Klimawandel`). Ist kein Bildmodell eingerichtet, entstehen Textfolien mit Hinweis. *Hinweis: über die lokale Z-Image-Brücke dauert jedes Bild ~30–50 s.* |
