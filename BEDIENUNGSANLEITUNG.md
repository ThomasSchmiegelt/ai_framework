...
| `/such <Begriff>` · `/suche` · `/finde` · `/search` | **Erweiterte Suche:** die KI erzeugt **alternative Suchbegriffe** (Synonyme, Fach-/Umgangssprache, engl. Begriffe) als anklickbare Chips, durchsucht damit das Web und fasst die Treffer **mit Quellen** zusammen. Hilfreich, wenn man den treffenden Fachbegriff nicht kennt. *(Hinweis: Ist beim `/finde` ein **Bild angehängt**, wird stattdessen die **Bilderkennung** ausgelöst — siehe nächste Zeile.)* |
...
| `/ziel <Beschreibung>` · `/goal` · `/zi` | **Ziel-Loop:** Du gibst **nur ein Ziel** vor — **keine Schritte**. Die KI zerlegt es selbst in Teilziele und arbeitet dann **in Runden** darauf hin: In jeder Runde erarbeitet sie den nächsten Fortschritt und **prüft anschließend selbst**, ob das Ziel schon erreicht ist. Ist es erreicht, hört sie auf; sonst geht sie mit der noch offenen Lücke in die nächste Runde — bis zum Ziel **oder** bis zum Runden-Deckel. Ein **Fortschrittsbalken** zeigt, wie weit sie ist; jede Runde ist zum Aufklappen, am Ende steht ein **Gesamtergebnis** mit den Buttons **→ Präsentation** und **→ Planer**. Optionale Marke am Anfang: `[web]` (jede Runde vorher im Internet recherchieren) und `[r7]` (Runden-Deckel, Standard 5, 2–10) — auch kombiniert `[web,r6]`. Beispiel: `/ziel [web,r6] Erarbeite einen Vergleich von drei Akku-Chemien für ein Lasten-E-Bike mit Empfehlung`. Ideal, wenn du **weißt, was herauskommen soll, aber nicht, welche Schritte nötig sind** (im Gegensatz zu `/workflow`, wo du die Schritte vorgibst). |
...
auf den Browser zurückgefallen. In dem Fall erscheint ein **kurzer Hinweis** („API-Sprachausgabe
fehlgeschlagen …"), damit du siehst, warum trotz gewähltem Modell die Browserstimme läuft — ein
häufiger Grund: **nicht jeder Anbieter unterstützt Sprachausgabe** (z. B. reine Text-Anbieter wie
OpenRouter/Groq haben kein `/audio/speech`). Wähle dann einen Anbieter mit TTS (etwa OpenAI).
